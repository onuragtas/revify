 ...
 */
if (app.isPackaged && !process.env.REVIFY_ENV)
    process.env.REVIFY_ENV = 'production';
...
async function setupUpdates() {
    ...
    const publish = (state) => server.setUpdateState({ supported: true, canInstall, current, ...state });
    publish({ status: 'idle' });
    ...
    autoUpdater.on('update-downloaded', (info) => {
        ...
        downloadedArchive = info.downloadedFile ?? null;
    ...
    server.setUpdateState({ supported: true, canInstall, current, status: 'idle' }, () => {
        ...
        void (async () => {
            ...
            try {
                ...
                notify('Güncelleme kurulamadı', `${message}\nİndirme sayfasından kurabilirsin.`, () => void shell.openExternal(releasesUrl()));
            }
        })();
    });
    const check = () => {
    ...
    app.on('browser-window-focus', () => {
        const now = Date.now();
        if (now - lastFocusCheck < 15 * 60 * 1000)
            return;
        lastFocusCheck = now;
        void check();
    });
...
if (!app.requestSingleInstanceLock()) {
    ...
    void app.whenReady().then(() => {
        ...
        server.events.on('fix:ready', ({ issueKey, files }) => {
            notify('Yama hazır', `${issueKey}\n${files} dosya değişti — okuyup kendi dizinine uygulayabilirsin.`, () => openIssue(issueKey));
        });
        ...
        server.events.on('review:auto-queued', ({ issueKey, summary, position }) => {
            notify(position === 0 ? 'Yeni iş inceleniyor' : 'Yeni iş sıraya alındı', `${issueKey}${summary ? ` — ${summary}` : ''}`, () => openIssue(issueKey), true);
        });
        server.events.on('update:installing', ({ version }) => {
            ...
            notify('Güncelleme kuruluyor', `Revify ${version} kuruluyor, uygulama birazdan yeniden başlayacak.`, () => showWindow());
        });
